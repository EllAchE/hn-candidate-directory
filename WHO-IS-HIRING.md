...
- `GET /api/candidates` reads only revisions whose status is `published` and whose Hacker News ingest record, when one exists, is not suppressed. `submitted`, `processing`, `review_ready`, `ingested`, `archived`, `failed`, and suppressed records cannot enter public search.
