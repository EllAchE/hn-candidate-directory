...
This repository is building a searchable directory from Hacker News “Who wants to be hired?” posts. Two ingestion paths feed it. Candidate-controlled submission covers private source-text, plain-text resume, and allowlisted LinkedIn URL ingestion, candidate review, explicit consent, refusal, publication, and token-verified update/removal management. Scheduled Hacker News ingestion publishes public thread comments directly, each linked back to the source comment, with a suppression record that a later removal request can set. Outbound email delivery is intentionally not part of this slice.

## What works

The same-origin Cloudflare Worker serves the static directory and eight API route groups:
...
- `POST /api/submissions/resume` accepts the raw bytes of one UTF-8 `text/plain` `.txt` resume with a simple ASCII filename. It enters the decoded text into the same private D1 and Queue path as source text, and returns no filename or source content. PDF, Word, HTML, archives, executables, and other binary formats are not supported.
...
- `POST /api/admin/ingest/hn` runs the Hacker News ingest on demand for an operator holding the `HN_INGEST_TOKEN` secret. It performs the same discovery and enqueue work as the scheduled trigger and returns only counts.
- `GET /api/candidates` reads only revisions whose status is `published` and whose Hacker News ingest record, when one exists, is not suppressed. `submitted`, `processing`, `review_ready`, `ingested`, `archived`, `failed`, and suppressed records cannot enter public search. Each public candidate includes a `processed` boolean: candidate-reviewed submissions are processed by definition, while Hacker News profiles become processed after the strongest registered extractor has handled them.
- `GET /api/candidates/stats` summarizes that same bounded public cohort with candidate, processed-profile, distinct-location, and distinct-university totals. Its per-facet coverage counts report how many profiles provide usable data for each directory filter.
- `POST /api/feedback` stores one pilot report — message, optional contact, optional candidate id — and returns only `received`. Nothing reads the table back over HTTP, so a report naming a person cannot become a page; an operator reads it with `wrangler d1 execute`. It is rate limited per client address like every other write.

The Queue consumer extracts normalized locations, work modes, availability, universities, companies, skills, and date ranges. One deterministic safety policy redacts extracted sensitive values before any source becomes a private review draft. It clears D1 source text after successful processing, then acknowledges redelivery of an already-ready draft without replacing it. The browser polls for the private draft and renders an editable review form. Saving remains separate from the consent checkbox and publish action. The raw management token is returned once, kept only in page memory, and never written to `localStorage`; D1 stores only its SHA-256 hash. Candidates may use that token to reopen a published profile for private editing or archive it. When the API is unavailable, the static demo uses the same policy for an in-tab source-text preview that cannot publish or manage a listing; URL and resume ingestion have no browser-side fallback.
...
Raw source handling is unchanged. D1 source text is private and cleared after successful extraction, so uploaded resume content is retained only for as long as extraction and Queue retry require. The redacted fields, rather than the raw source, enter review and public serialization.

## Resume upload controls

Resume uploads use a raw request body so the Worker can reject a declared `Content-Length` above 100,000 bytes before reading and stop a chunked body as soon as buffering crosses the same limit. The endpoint requires `text/plain` with an optional UTF-8 charset plus a single-extension ASCII `.txt` filename. The filename is validated but never persisted.
...
SHA-256 of the exact file bytes derives the D1 submission ID. The D1 primary key establishes the race-safe deduplication winner, so concurrent duplicate losers never receive a review token.

D1 is the sole store for validated resume text; the deployment uses no object storage. The existing `source_kind = 'text'` row lets the current Queue consumer and a rolled-back Worker process the text without a migration. Successful extraction clears it. Processing failures retain it for Queue retry; setup and Queue-send failures return only stable errors.
...
A six-hourly cron trigger is the production ingestion path; it is the only path that can write D1 without an operator mutation. `POST /api/admin/ingest/hn` runs the same work immediately so the first ingest need not wait for the schedule, and `scripts/ingest-hn.js` previews extraction coverage for a thread without touching D1 at all. Every network call goes through an injectable transport, so no test reaches Hacker News.

Discovery searches the Algolia index for recent `whoishiring` “who wants to be hired” stories, then pages that story's comments. Only top-level comments are considered; replies are dropped. Each comment becomes one Queue message, and the Queue consumer performs the D1 writes, so a slow thread cannot exceed a request's CPU budget.

Extraction is deterministic. `DETERMINISTIC_HN_EXTRACTOR` decodes the comment HTML — anchors collapse to their `href`, block tags become line breaks, entities are decoded — then reads the conventional `Location:`, `Remote:`, `Willing to relocate:`, `Technologies:`, `Companies:`, `Education:`, `Availability:`, `Résumé/CV:`, and `Email:` labels, matching accents and casing insensitively. Unmapped lines become the summary, and a comment with no labels at all falls back to prose: the leading sentence becomes the role and remote intent is read from the text. A field with no evidence is recorded as `Not specified` rather than guessed, and a comment with neither labels nor enough prose is recorded as ingested but produces no profile. The extractor is passed in as `{ id, extract(record) }`, so a different implementation can replace it without touching ingestion, idempotency, or storage.

Every extracted draft passes through the shared sensitive-data policy before storage, and again at public serialization. Hacker News comments carry email addresses and phone numbers constantly; those reach D1 already redacted.

Re-running ingestion is safe. Each comment's `hn_ingests` row is keyed on the immutable Hacker News item id and stores a hash of the decoded text: an unchanged comment is skipped before it is queued, and an edited one updates the existing profile revision in place rather than creating a second. The submission ID is derived from the item id, so the D1 primary key backstops deduplication even if a message is redelivered. An `archived` revision is never returned to `published` by an update.
