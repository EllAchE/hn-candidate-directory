...
# remote: one isolated extraction process per batch, four at a time, under tmux so it
# outlives the ssh channel
export HNCD_EXTRACTOR=codex   # omit this line to use Claude Code
tmux new-session -d -s hncd-<tag> "seq 1 <n> | HNCD_EXTRACTOR=$HNCD_EXTRACTOR xargs -P 4 -I{} ./scripts/extract-hn-profiles/devbox-extract-batch.sh {}"
